...
const palettes = {
	green: {
		...
		houseColor: 0x8b4513, // Brown for houses
	...
	orange: {
		...
		rockColor: 0xacacac,
		treeColor: 0xa2d109,
		houseColor: 0xd2691e, // Chocolate brown for houses
		candyColor: 0x614bdd,
		snakeColor: 0xff470a,
		mouthColor: 0x614bdd,
	},
	lilac: {
		...
		rockColor: 0xebebeb,
		treeColor: 0x53d0c1,
		houseColor: 0xff69b4, // Pink for houses
		candyColor: 0x9900ff,
		snakeColor: 0xff2ed2,
		mouthColor: 0x614bdd,
	},
...
function applyPalette(paletteName) {
	...
	entities
		.find((entity) => entity instanceof House)
		?.mesh.children[0].material.color.set(houseColor)
...
// Add decorative houses around the scene (background)
const houseData = [
	// Left side
	...
	// Right side
	new THREE.Vector3(resX + 6, 0, 3),
	//new THREE.Vector3(resX + 8, 0, 9),
	//new THREE.Vector3(resX + 7, 0, 15),
	new THREE.Vector3(resX + 9, 0, 21),

	// Top/Far side
	new THREE.Vector3(2, 0, -8),
	//new THREE.Vector3(8, 0, -10),
	//new THREE.Vector3(14, 0, -9),
	new THREE.Vector3(resX - 3, 0, -7),

	// Bottom/Near side
	new THREE.Vector3(3, 0, resX + 6),
	new THREE.Vector3(10, 0, resX + 8),
	new THREE.Vector3(resX - 5, 0, resX + 7),
]

const house = new House(resolution)

houseData.forEach((position) => {
	let clone = house.mesh.clone(true)
	clone.position.copy(position)
	clone.scale.setScalar(1.5 + Math.random() * 1.0)  // Bigger houses (1.5x to 2.5x)
	clone.rotation.y = Math.random() * Math.PI * 2
	clone.castShadow = true
	scene.add(clone)
})

// Add Obelix characters in the background
const obelixData = [
	
	new THREE.Vector3(resX + 4, 0, 12),
	
]

obelixData.forEach((position) => {
	const obelix = new Obelix(resolution, null, (model) => {
		// Position and scale the loaded model
		obelix.mesh.position.copy(position)
		obelix.mesh.scale.setScalar(2 + Math.random() * 0.5)  // Scale 2x to 2.5x
		obelix.mesh.rotation.y = Math.random() * Math.PI * 2
	})
	scene.add(obelix.mesh)
})
